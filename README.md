...
- `06:05`：在 DeepSeek 闲时时段执行完整采集，目标 40 条。
- `07:35`：只检查并补足实际缺口；已经达标则不调用 DeepSeek。
- `12:10`：午间补量检查，只处理仍缺少的条目。
- `13:10`：最后一次缺口兜底；已经达标则立即退出。
- DeepSeek 只允许在 `06:00-08:30` 和 `12:01-13:59` 发起请求，代码另留 3 分钟收尾缓冲。GitHub 延迟或高峰期手动触发时也会被硬门禁拦截。
