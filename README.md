...
- DeepSeek 先做大规模文字预筛；工作日白天在线的 Mac 再用公司 GPT-5.5 检查图片、品类和创新证据。
...
-> 全库去重
-> DeepSeek 严格品类终审
-> 公司 GPT-5.5 多模态复核图片匹配、实体品类和创新质量
-> 不足 40 条时换搜索角度继续补量
-> 评分、趋势与周报
-> GitHub Pages 更新
-> 飞书 Top 5 推送
...
- `02:30`：在 DeepSeek 闲时时段执行完整采集，目标 40 条。
- `05:30`：第一次缺口检查；已经达标则不调用 DeepSeek。
- `08:00`：早间最后一次缺口检查，并为 GitHub 调度延迟留出余量。
- `12:10`：午间补量检查，只处理仍缺少的条目。
- `13:10`：最后一次缺口兜底；已经达标则立即退出。
- DeepSeek 只允许在官方空闲时段 `00:00-08:59`、`12:01-13:59` 和 `18:01-23:59` 发起请求，代码另留 3 分钟收尾缓冲。GitHub 延迟或高峰期手动触发时也会被硬门禁拦截。
- 只要当天不足 40 条，每轮都维持完整候选搜索规模，再由严格品类与质量门槛筛选；不设置日常 DeepSeek 调用配额。当天满 40 条后自动停止，不会为了刷新排序重复搜索。
...
各阶段只收集数据库里从未出现过的新内容，不会把旧池内容改成今天日期。
...
COMPANY_GPT_API_KEY=你的公司 API Key
...
```bash
python3 scripts/company_multimodal_review.py --date 2026-07-20 --workers 3
```
