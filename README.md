...
- 每天北京时间 08:30、13:30、17:30 分三次自动收集公开设计媒体、产品媒体、市场信号和社媒公开索引结果。
...
- 每天生成一个日期分组，默认展示当天前 30 条去重情报，其中优先保证 10 条来自抖音、小红书、Instagram 的公开内容页。
...
- 抖音公开内容页
- 小红书公开笔记页
- Instagram 公开帖子和 Reel

社媒来源只通过公开搜索索引发现具体内容页，不登录账号、不绕过平台限制。搜索页、用户页、话题页不会进入数据池。
...
- 08:30：当天目标 10 条，其中社媒公开索引目标 4 条。
- 13:30：当天目标 20 条，其中社媒公开索引目标 7 条。
- 17:30：当天目标 30 条，其中社媒公开索引目标 10 条。

每次都会先继续收集新线索，再做去重和审核；如果当前阶段的抖音/小红书/Instagram 公开内容页不足，会优先补采社媒公开索引；最后再按阶段目标刷新页面。手动运行用于修复当天数据，会直接按 30 条和 10 条社媒目标执行。
...
注意：公开索引不是平台内全量搜索，只能覆盖被搜索引擎收录的公开内容。系统会把社媒公开索引设为每日优先配额，但如果当天公开索引确实找不到足够多的真实内容页，脚本会保留真实结果并打印 warning，不会伪造链接或把搜索页当成产品。
