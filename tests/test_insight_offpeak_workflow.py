...
class InsightOffPeakWorkflowTests(unittest.TestCase):
    def test_schedules_stay_inside_deepseek_windows(self):
        ...
        self.assertIn('DEEPSEEK_MIN_ROUND_MINUTES: "35"', workflow)
        self.assertIn('--agent-batch-size 12', workflow)
        self.assertIn('--min-round-minutes "$DEEPSEEK_MIN_ROUND_MINUTES"', workflow)
